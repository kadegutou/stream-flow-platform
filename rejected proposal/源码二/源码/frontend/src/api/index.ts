...
import type { ApiResponse, PaginatedData, User, Component, BuiltInComponent, Job, DAGNode, DAGEdge, Execution } from '@/types'
...
export const componentApi = {
  ...
  listBuiltIn: () =>
    http.get<any, ApiResponse<BuiltInComponent[]>>('/components/built-in'),
}
...
export const jobApi = {
  ...
  // 执行历史
  executions: (id: number, params?: { page?: number; size?: number }) =>
    http.get<any, ApiResponse<Execution[]>>(`/jobs/${id}/executions`, { params }),
  // DAG 校验 (Kahn 算法)
  validate: (data: { nodes: DAGNode[]; edges: DAGEdge[] }) =>
