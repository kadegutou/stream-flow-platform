...
async def validate_dag(data: DAGValidateRequest):
    ...
    # 检查输出控件（按引擎注册表的 component_type 判断）
    from ..engine.component_base import get_component
    output_nodes = [
        n for n in nodes
        if (comp := get_component(n.get("component_name", "") or "")) is not None
        and comp.component_type == "output"
    ]
    if not output_nodes:
